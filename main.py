...
When asked who you are, what model you are, or who created you, always identify yourself as Phoenix AI developed by Phoenix Teams & IYANU.
You are running on self-hosted infrastructure powered by Ollama and FastAPI.
Never mention Qwen, Alibaba, OpenAI, Anthropic, Google, or any other company as your creator.
Your identity is Phoenix AI.""")
...
try:
    ...
    if "sslmode=require" not in _db_url and "postgresql" in _db_url:
        _db_url += "&sslmode=require" if "?" in _db_url else "?sslmode=require"
    
    engine = create_engine(_db_url, pool_pre_ping=True, connect_args={"connect_timeout": 10, "sslmode": "require"})
...
def chat_completions(
    ...
    # We use Azure if it's configured AND (either the model matches the deployment name or no specific model was requested but Azure is preferred)
    use_azure_for_this = USE_AZURE and (req.model == AZURE_OPENAI_DEPLOYMENT_NAME or not req.model)
    
    try:
        if use_azure_for_this:
            # Prepare messages with Phoenix system prompt
            messages = [{"role": "system", "content": PHOENIX_SYSTEM_PROMPT}]
            ...
            if req.stream:
                ...
                def azure_streamer():
                    ...
                    for chunk in response:
                        if chunk.choices and len(chunk.choices) > 0:
                            ...
                            # Format to match the expected streaming output if needed, 
                            # but here we just pass the JSON string of the chunk
                    ...
                    log_usage(db, auth["id"], auth.get("id") if auth["type"] == "api_key" else None, 
                             "/v1/chat/completions", AZURE_OPENAI_DEPLOYMENT_NAME, tokens, status="success", duration_ms=duration)
                    trigger_webhooks(auth["id"], "chat.completed", {"model": AZURE_OPENAI_DEPLOYMENT_NAME, "tokens": tokens}, db)
                ...
                duration = (time.time() - start_time) * 1000
                log_usage(db, auth["id"], auth.get("id") if auth["type"] == "api_key" else None,
                         "/v1/chat/completions", AZURE_OPENAI_DEPLOYMENT_NAME, total_tokens, status="success", duration_ms=duration)

                background_tasks.add_task(trigger_webhooks, auth["id"], "chat.completed", 
                                         {"model": AZURE_OPENAI_DEPLOYMENT_NAME, "tokens": total_tokens, "preview": content[:100]}, db)
            ...
            # Fallback to Ollama
            # Inject Phoenix AI identity as system prompt
            messages = [{"role": "system", "content": PHOENIX_SYSTEM_PROMPT}]
            for m in req.messages:
                messages.append({"role": m.role, "content": m.content})
        ...
        provider = "Azure" if use_azure_for_this else "Ollama"
...
<tr><td>OLLAMA_HOST</td><td>Internal Ollama URL</td></tr>
	...
	<tr><td>AZURE_OPENAI_ENDPOINT</td><td>Azure OpenAI endpoint URL</td></tr>
	<tr><td>AZURE_OPENAI_KEY</td><td>Azure OpenAI API Key</td></tr>
	<tr><td>AZURE_OPENAI_DEPLOYMENT_NAME</td><td>Azure Deployment Name</td></tr>
	<tr><td>AZURE_OPENAI_API_VERSION</td><td>Azure API Version (default: 2024-02-15-preview)</td></tr>
	</table>
</div>
